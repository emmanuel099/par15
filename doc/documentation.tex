...
If we ignore the overhead of the cilk instructions the runtime of this algorithm should be in the order of $O(\frac{n \cdot m}{p})$, where $m$ denotes the number of columns, $n$ the number of rows and $p$ the number of wokers.
...
The matrix is distributed among the available threads p, by splitting the matrix horizontally, assigning each worker $\frac{n}{p}$ rows. Each worker buffers the result of the last row (to ensure that the other threads use the correct boundary) and uses an another row buffer for the calculation of the following rows (similar to algorithm \ref{algo:seq_one_vec}).\\
At the end of each iteration the last row buffer is copied back to the original array.\\
The memory consumption of this algorithm is $O(n \cdot m + 2 \cdot p \cdot m)$ and the runtime is $O(\frac{m \cdot n}{p})$

\begin{algorithm}[H] \label{algo:cilk}
 \KwData{A: input 2D array, it: iterations, thread: current thread number}
 \Parallel{}{
    start\_row = thread $\cdot$ rows\_per\_thread\\
    end\_row = start\_row + rows\_per\_thread - 1\\
    
    \ForEach{iteration}{
	row\_buffer = five\_point\_stencil\_for\_row(A, start\_row)\\
	last\_row\_buffer = five\_point\_stencil\_for\_row(A, end\_row)\\
	\#pragma omp barrier

	\For{r = start\_row + 1 to end\_row}{
	  \ForEach{column c}{
	    value = stencil\_five\_point\_kernel(A, r, c)\\
	    A[r - 1, c] = row\_buffer[c]\\
	    row\_buffer[c] = value
	  }
	}
	
	\ForEach{column c}{
	  A[end\_row - 1, c] = row\_buffer[c]\\
	  A[end\_row, c] = last\_row\_buffer[c]
	}
	\#pragma omp barrier
    }
 }
...
\begin{algorithm}[H] \label{algo:cilk}
 ...
 \Parallel{}{
	...
	row\_buffer = five\_point\_stencil\_for\_row(B, start\_row)\\         
	
	\For{r = start\_row + 1 to end\_row}{
	  \ForEach{column c}{
	    value = stencil\_five\_point\_kernel(B, r, c)\\
	    B[r - 1, c] = row\_buffer[c]\\
	    row\_buffer[c] = value
	  }
	}
	\ForEach{column c}{
	  B[end\_row - 1, c] = row\_buffer[c]\\
	}
    } 
 }
