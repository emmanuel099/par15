...
We can see, as assumed, the row-wise implementations outperform the column-wise implementations rather significant, due to the lower caching overhead. Furthermore the memory-inefficient versions (tld) perform much better than their counterparts. Creating submatrices on each nodes does have the downsides of using about twice the amount of memory and a higher runtime complexity due to the exchange of the boundary data, but due to the locality of the submatrix data the memory access is much faster, thus overcompensating the downsides and resulting in better performance.
